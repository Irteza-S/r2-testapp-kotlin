...
var currentChapterSearchResult = [];
...
function moveToNextWord() {
    console.log("next word");
    	if(currentPosition<currentChapterSearchResult.length - 1) {
      	jumpToMark(currentPosition + 1);
        currentPosition = currentPosition + 1 ;
      } else {
        //Next resource
        //window.MyJSClient.goToNextResource(currentResource);
      }
    console.log("Current position : "+ currentPosition);
};
...
function jumpToMark(index) {
       ...
       if (currentChapterSearchResult.length > 0) {
           var position
           var current = currentChapterSearchResult[index];
           for (var i = 0; i < currentChapterSearchResult.length; i++) {
               currentChapterSearchResult[i].classList.remove(currentClass);
           }
...
function scrollToElement(element) {
    var elementOffset = element.scrollRight // element.getBoundingClientRect().left works for Gutenbergs books
    var offset = Math.round(window.scrollX + elementOffset);

    document.scrollingElement.scrollRight = snapOffset(offset);
};


function ok() {
    window.MyJSClient.getStringFromJS("Im IN");
};

function performMarkInCurrentResource(resource, keyword, focusPosition) {
    ...
    markInstance.mark(keyword, {
               each: function(node){
                    //console.log(node);
                    currentChapterSearchResult.push(node);
                },
              done: function(counter){
                //window.MyJSClient.getStringFromJS("MARKED NB : "+counter);
        	}
           });
     //console.log(currentChapterSearchResult);
     currentPosition = 0;
     jumpToMark(focusPosition);
    //window.MyJSClient.getStringFromJS("MARKING");

};


function performMark(keyword, element, resourceId) {

  //window.MyJSClient.getStringFromJS("Im IN");
  console.log(resourceId);
  currentChapterSearchResult.length = 0;
  ...
  //var markInstance = new Mark(document.querySelector(".text"));
  //console.log(document.querySelector(".text").innerHTML);
  //console.log("performing search on : "+keyword);
  //console.log("data : "+markInstance);
  // Determine selected options
    ...
    markInstance.mark(keyword, {
              each: function(node){
    						...
    						//console.log(node);
    						var tmp = "";
    						var prev = node.previousSibling.nodeValue
                            var next = node.nextSibling.nodeValue

                            if (next.length > 30) next = next.substring(0, 30) + "...";
                            if (prev.length > 30) prev = "..." + prev.substring(prev.length - 30, prev.length);

                            tmp = prev + "[" + keyword + "]" + next;
    ...
    markInstance.mark(keyword, {
     ...
     if(found) {
         ...
         //console.log(myObj);
         var resultsString = JSON.stringify(myObj);
         console.log("ResultsString : "+ resultsString);
         //window.MyJSClient.getResults(resultsString);
         return myObj;
     }
...
$(function() {
    ...
    // the input field
    var $searchBtn = $("button[data-search='searchBtn']"),
